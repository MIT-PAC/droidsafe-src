    ...
    public static <T> T runWithTimeout(Callable<T> callable, long timeout, TimeUnit timeUnit) 
        ...
        if (timeout == 0) {
            ...
            try {
                ...
                //cancel here just in case the exception is not thrown
                future.cancel(true);
            } catch (Exception e) { //catch all exceptions and cancel the future
                //cancel the future for all further processing
                future.cancel(true);
                throw e;
            }
            return ret;
