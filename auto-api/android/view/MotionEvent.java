...
public final class MotionEvent extends InputEvent implements Parcelable {
	...
	private static final String[] BUTTON_SYMBOLIC_NAMES = new String[] {
			...
			"BUTTON_BACK", "BUTTON_FORWARD", "0x00000020", "0x00000040",
	...
	public static final Parcelable.Creator<MotionEvent> CREATOR = new Parcelable.Creator<MotionEvent>() {
		...
		public MotionEvent createFromParcel(Parcel in) {
			...
			in.readInt();
			MotionEvent varDBE57DA48AA46F9897DCADB263EB85B9_941366718 = (MotionEvent
	...
	private static final void ensureSharedTempPointerCapacity(
			int desiredCapacity) {
		if (gSharedTempPointerCoords == null
			...
			gSharedTempPointerProperties = PointerProperties
	...
	private static int nativeInitialize(int nativePtr, int deviceId,
			int source, int action, int flags, int edgeFlags, int metaState,
			int buttonState, float xOffset, float yOffset, float xPrecision,
			float yPrecision, long downTimeNanos, long eventTimeNanos,
			int pointerCount, PointerProperties[] pointerIds,
	...
	private static void nativeOffsetLocation(int nativePtr, float deltaX,
			float deltaY) {
		// DSFIXME: CODE0010: Native static method requires manual modeling
	}
	...
	private static float nativeGetRawAxisValue(int nativePtr, int axis,
	...
	private static float nativeGetAxisValue(int nativePtr, int axis,
			int pointerIndex, int historyPos) {
		// DSFIXME: CODE0010: Native static method requires manual modeling
		return DSUtils.UNKNOWN_FLOAT;
	}
	...
	private static void nativeGetPointerCoords(int nativePtr, int pointerIndex,
			int historyPos, PointerCoords outPointerCoords) {
		// DSFIXME: CODE0010: Native static method requires manual modeling
	...
	static public MotionEvent obtain(long downTime, long eventTime, int action,
		...
		ev.mNativePtr = nativeInitialize(ev.mNativePtr, deviceId, source,
				action, flags, edgeFlags, metaState, buttonState, 0, 0,
				xPrecision, yPrecision, downTime * NS_PER_MS, eventTime
						* NS_PER_MS, pointerCount, pointerProperties,
				pointerCoords);
		return ev;
	}
	...
	static public MotionEvent obtain(long downTime, long eventTime, int action,
		...
		synchronized (gSharedTempLock) {
			...
			return obtain(downTime, eventTime, action, pointerCount, pp,
					pointerCoords, metaState, 0, xPrecision, yPrecision,
	...
	static public MotionEvent obtain(long downTime, long eventTime, int action,
		...
		synchronized (gSharedTempLock) {
			...
			pc[0].size = size;
			ev.mNativePtr = nativeInitialize(ev.mNativePtr, deviceId,
					InputDevice.SOURCE_UNKNOWN, action, 0, edgeFlags,
					metaState, 0, 0, 0, xPrecision, yPrecision, downTime
							* NS_PER_MS, eventTime * NS_PER_MS, 1, pp, pc);
			return ev;
		}
	}
	...
	@Deprecated
	static public MotionEvent obtain(long downTime, long eventTime, int action,
			int pointerCount, float x, float y, float pressure, float size,
			int metaState, float xPrecision, float yPrecision, int deviceId,
			int edgeFlags) {
	...
	static public MotionEvent obtain(long downTime, long eventTime, int action,
		...
		return obtain(downTime, eventTime, action, x, y, 1.0f, 1.0f, metaState,
				1.0f, 1.0f, 0, 0);
	}
	...
	static public MotionEvent obtain(MotionEvent other) {
		if (other == null) {
			throw new IllegalArgumentException(
					"other motion event must not be null");
		}
		MotionEvent ev = obtain();
	...
	public final int getActionIndex() {
		// DSFIXME: CODE0009: Possible callback target function detected
		int var853FB894ACCBEFFE5F8E24DD3377AFAF_1285366496 = ((nativeGetAction(mNativePtr) & ACTION_POINTER_INDEX_MASK) >> ACTION_POINTER_INDEX_SHIFT);
		return dsTaint.getTaintInt();
		// ---------- Original Method ----------
	...
	public final void setTainted(boolean tainted) {
		...
		final int flags;
		flags = getFlags();
		nativeSetFlags(mNativePtr, tainted ? flags | FLAG_TAINTED : flags
				& ~FLAG_TAINTED);
		// ---------- Original Method ----------
		// final int flags = getFlags();
		// nativeSetFlags(mNativePtr, tainted ? flags | FLAG_TAINTED : flags &
	...
	public final long getEventTime() {
		long varABD7EED75F03AC822986337AA0AEBA8E_769939818 = (nativeGetEventTimeNanos(
				mNativePtr, HISTORY_CURRENT) / NS_PER_MS);
		return dsTaint.getTaintLong();
		// ---------- Original Method ----------
	...
	public final long getEventTimeNano() {
		long var9177F0DF4886BA63EBD71602211F864C_1162485415 = (nativeGetEventTimeNanos(
				mNativePtr, HISTORY_CURRENT));
		return dsTaint.getTaintLong();
		// ---------- Original Method ----------
		// return nativeGetEventTimeNanos(mNativePtr, HISTORY_CURRENT);
	}
	...
	public final float getX() {
		float var66044244940F058BEDF52F995ECDF659_37813940 = (nativeGetAxisValue(
				mNativePtr, AXIS_X, 0, HISTORY_CURRENT));
	...
	public final float getY() {
		float varC75AFE4F07E18DED73B6E9A135DBAA24_1796179246 = (nativeGetAxisValue(
				mNativePtr, AXIS_Y, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getPressure() {
		float varEF9A2F87450786CEB865F6DB8CC3ECFD_358613681 = (nativeGetAxisValue(
				mNativePtr, AXIS_PRESSURE, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_PRESSURE, 0,
		// HISTORY_CURRENT);
	}
	...
	public final float getSize() {
		float varDFF5D0290CDD1A07C8B97F2F6E962921_495059519 = (nativeGetAxisValue(
				mNativePtr, AXIS_SIZE, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getTouchMajor() {
		float var2C0DAF19E2431B7454CC6F77C73821E0_771585223 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MAJOR, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOUCH_MAJOR, 0,
		// HISTORY_CURRENT);
	}
	...
	public final float getTouchMinor() {
		float var4508F4DDA7AD6F23C2A12D53FC2B6353_1662880813 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MINOR, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOUCH_MINOR, 0,
		// HISTORY_CURRENT);
	}
	...
	public final float getToolMajor() {
		float varC9DDB5D5521C8A6BB646AB1BF9C7D9E2_331474788 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MAJOR, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOOL_MAJOR, 0,
		// HISTORY_CURRENT);
	}
	...
	public final float getToolMinor() {
		float var695C0E240E818E6BDB2EBB725DE208C8_1988395379 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MINOR, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOOL_MINOR, 0,
		// HISTORY_CURRENT);
	}
	...
	public final float getOrientation() {
		float var75B1DB96187E89974E64BC0FB38B1FEF_1095148901 = (nativeGetAxisValue(
				mNativePtr, AXIS_ORIENTATION, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_ORIENTATION, 0,
		// HISTORY_CURRENT);
	}
	...
	public final float getAxisValue(int axis) {
		...
		float var4FF16B9B17710DEACD9EEA98B643B740_743275898 = (nativeGetAxisValue(
				mNativePtr, axis, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final int getPointerId(int pointerIndex) {
		...
		int var17A8C53C4A6DAF94DA4FE0E64E276DEB_1904343743 = (nativeGetPointerId(
				mNativePtr, pointerIndex));
		return dsTaint.getTaintInt();
		// ---------- Original Method ----------
	...
	public final int getToolType(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		int var705074F16347B844E833B41BDB330DC5_443153741 = (nativeGetToolType(
				mNativePtr, pointerIndex));
		return dsTaint.getTaintInt();
		// ---------- Original Method ----------
	...
	public final int findPointerIndex(int pointerId) {
		...
		int varCF3227509A91EFEBFB983EC37BCBBB09_1305949257 = (nativeFindPointerIndex(
				mNativePtr, pointerId));
		return dsTaint.getTaintInt();
		// ---------- Original Method ----------
	...
	public final float getX(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float var22607ACD4999ED3178C2C16BA32F39FA_1639088007 = (nativeGetAxisValue(
				mNativePtr, AXIS_X, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_X, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getY(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float varEC964DA0D0FCB932F45F9605581B800C_2075181069 = (nativeGetAxisValue(
				mNativePtr, AXIS_Y, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_Y, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getPressure(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float varDB60B99C5EF7895E6B92B91772C5962E_327975946 = (nativeGetAxisValue(
				mNativePtr, AXIS_PRESSURE, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_PRESSURE, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getSize(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float varB4E63BA449D57F4253A0AB8919D42D10_1189148399 = (nativeGetAxisValue(
				mNativePtr, AXIS_SIZE, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_SIZE, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getTouchMajor(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float varA24CED3D0F358E3C65B964973B4F5F83_1330471154 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MAJOR, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOUCH_MAJOR, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getTouchMinor(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float var120D3EDA1AEC95857772396D3ADDF431_1945342792 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MINOR, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOUCH_MINOR, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getToolMajor(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float varE1A0953AB1720A635D0A001029F15F98_1612322673 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MAJOR, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOOL_MAJOR, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getToolMinor(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float varC32F5653C2BD206ECB16EB3579C5B644_1888729241 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MINOR, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOOL_MINOR, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getOrientation(int pointerIndex) {
		dsTaint.addTaint(pointerIndex);
		float var865B68C13F122391AB0E054D3314ED41_335683619 = (nativeGetAxisValue(
				mNativePtr, AXIS_ORIENTATION, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_ORIENTATION, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final float getAxisValue(int axis, int pointerIndex) {
		dsTaint.addTaint(axis);
		dsTaint.addTaint(pointerIndex);
		float varDC8E493A6FB2EE0C0CB23BC5DBBC02F1_1568175215 = (nativeGetAxisValue(
				mNativePtr, axis, pointerIndex, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, axis, pointerIndex,
		// HISTORY_CURRENT);
	}
	...
	public final void getPointerProperties(int pointerIndex,
		...
		nativeGetPointerProperties(mNativePtr, pointerIndex,
	...
	public final float getRawX() {
		float var78851251BB2BADD4A73F875B41E82C95_975334394 = (nativeGetRawAxisValue(
				mNativePtr, AXIS_X, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getRawY() {
		float varB2F71D566D9DFEDA8765CB859668364C_491588550 = (nativeGetRawAxisValue(
				mNativePtr, AXIS_Y, 0, HISTORY_CURRENT));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final long getHistoricalEventTime(int pos) {
		...
		long var86B3FEF5B6798FC4F49A53A44960DD31_362684035 = (nativeGetEventTimeNanos(
				mNativePtr, pos) / NS_PER_MS);
		return dsTaint.getTaintLong();
		// ---------- Original Method ----------
	...
	public final float getHistoricalX(int pos) {
		dsTaint.addTaint(pos);
		float varD6254ED09F523E7C1E53ADA626DF0C10_1524971196 = (nativeGetAxisValue(
				mNativePtr, AXIS_X, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalY(int pos) {
		dsTaint.addTaint(pos);
		float varF753E500532420F185A79C4FC58F3F0B_695705340 = (nativeGetAxisValue(
				mNativePtr, AXIS_Y, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalPressure(int pos) {
		dsTaint.addTaint(pos);
		float var8CCA0683C3AD89698B4BB2DCB56AF6E0_1280149595 = (nativeGetAxisValue(
				mNativePtr, AXIS_PRESSURE, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalSize(int pos) {
		dsTaint.addTaint(pos);
		float varA2CCAFC013471427EED1D8795B79DF34_1471683617 = (nativeGetAxisValue(
				mNativePtr, AXIS_SIZE, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalTouchMajor(int pos) {
		dsTaint.addTaint(pos);
		float var8E7AD2DB9688820EB8F0B7CB347E27F7_372963472 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MAJOR, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalTouchMinor(int pos) {
		dsTaint.addTaint(pos);
		float var47136F660FFE6DBB3965B94BCAC103FF_116398409 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MINOR, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalToolMajor(int pos) {
		dsTaint.addTaint(pos);
		float varACE33102325AC5113F663D281165CB12_1233515162 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MAJOR, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalToolMinor(int pos) {
		dsTaint.addTaint(pos);
		float varBDFEBFC87FD2C86C951881656D01FBFE_272762466 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MINOR, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalOrientation(int pos) {
		dsTaint.addTaint(pos);
		float var9ABC98E782A3870F9AA5DFC3F27D891C_171035155 = (nativeGetAxisValue(
				mNativePtr, AXIS_ORIENTATION, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalAxisValue(int axis, int pos) {
		dsTaint.addTaint(axis);
		dsTaint.addTaint(pos);
		float varE580B6B92EAD96F426B34B55A944EC3F_260461949 = (nativeGetAxisValue(
				mNativePtr, axis, 0, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalX(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var70F4D2CDA260BE637BE2F366621D1047_1966032956 = (nativeGetAxisValue(
				mNativePtr, AXIS_X, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalY(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var4338B7951F95F5B1B66FDB2F4DC27413_784008045 = (nativeGetAxisValue(
				mNativePtr, AXIS_Y, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalPressure(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var599EE82AA7EED666174FF6603E5D9EDB_2003394302 = (nativeGetAxisValue(
				mNativePtr, AXIS_PRESSURE, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_PRESSURE, pointerIndex,
		// pos);
	...
	public final float getHistoricalSize(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var6ADB36223580AAF1DF6650FEF9BEFE5F_877924808 = (nativeGetAxisValue(
				mNativePtr, AXIS_SIZE, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final float getHistoricalTouchMajor(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var04576CC24CDC939EA1B84F18BA4AA27C_1758075808 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MAJOR, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOUCH_MAJOR, pointerIndex,
		// pos);
	...
	public final float getHistoricalTouchMinor(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float varE0E8C51F7817613AD2DBA52263E38B64_1854299260 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOUCH_MINOR, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOUCH_MINOR, pointerIndex,
		// pos);
	...
	public final float getHistoricalToolMajor(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var40EEA85674FBE73ABF2A2BD2F2E74CD6_1052717172 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MAJOR, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOOL_MAJOR, pointerIndex,
		// pos);
	...
	public final float getHistoricalToolMinor(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var784F21A01E3CEC7D68998B91D48C129C_312076320 = (nativeGetAxisValue(
				mNativePtr, AXIS_TOOL_MINOR, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_TOOL_MINOR, pointerIndex,
		// pos);
	...
	public final float getHistoricalOrientation(int pointerIndex, int pos) {
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var02FA0A42AAD0F907D9EB0C172EDB6AB4_2032572315 = (nativeGetAxisValue(
				mNativePtr, AXIS_ORIENTATION, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
		// return nativeGetAxisValue(mNativePtr, AXIS_ORIENTATION, pointerIndex,
		// pos);
	...
	// DSFIXME: CODE0002: Requires DSC value to be set
	public final float getHistoricalAxisValue(int axis, int pointerIndex,
			int pos) {
		dsTaint.addTaint(axis);
		dsTaint.addTaint(pointerIndex);
		dsTaint.addTaint(pos);
		float var675E97582C752387B4365682C4F7B0FA_1882865529 = (nativeGetAxisValue(
				mNativePtr, axis, pointerIndex, pos));
		return dsTaint.getTaintFloat();
		// ---------- Original Method ----------
	...
	public final void getHistoricalPointerCoords(int pointerIndex, int pos,
		...
		// ---------- Original Method ----------
		// nativeGetPointerCoords(mNativePtr, pointerIndex, pos,
		// outPointerCoords);
	}
	...
	@DSModeled(DSC.SAFE)
	public final void addBatch(long eventTime, PointerCoords[] pointerCoords,
			int metaState) {
		dsTaint.addTaint(metaState);
		dsTaint.addTaint(pointerCoords[0].dsTaint);
		dsTaint.addTaint(eventTime);
		nativeAddBatch(mNativePtr, eventTime * NS_PER_MS, pointerCoords,
				metaState);
		// ---------- Original Method ----------
		// nativeAddBatch(mNativePtr, eventTime * NS_PER_MS, pointerCoords,
		// metaState);
	...
	public final boolean isWithinBoundsNoHistory(float left, float top,
		...
		// for (int i = 0; i < pointerCount; i++) {
		// final float x = nativeGetAxisValue(mNativePtr, AXIS_X, i,
		// HISTORY_CURRENT);
		// final float y = nativeGetAxisValue(mNativePtr, AXIS_Y, i,
		// HISTORY_CURRENT);
	...
	// DSFIXME: CODE0002: Requires DSC value to be set
	public final MotionEvent clampNoHistory(float left, float top, float right,
			float bottom) {
		dsTaint.addTaint(bottom);
		dsTaint.addTaint(left);
		dsTaint.addTaint(right);
		...
		ev = obtain();
		{
			...
			pc = gSharedTempPointerCoords;
			{
				...
				i = 0;
				{
					...
					nativeGetPointerCoords(mNativePtr, i, HISTORY_CURRENT,
							pc[i]);
	...
	public final MotionEvent split(int idBits) {
		...
		ev = obtain();
		{
			...
			oldActionPointerIndex = (oldAction & ACTION_POINTER_INDEX_MASK) >> ACTION_POINTER_INDEX_SHIFT;
			...
			newIdBits = 0;
			{
				...
				i = 0;
				{
					nativeGetPointerProperties(mNativePtr, i,
							pp[newPointerCount]);
					final int idBit;
			...
			} // End collapsed parenthetic
			{
				if (DroidSafeAndroidRuntime.control)
					throw new IllegalArgumentException(
							"idBits did not match any ids in the event");
			} // End block
			final int newAction;
			{
				{
			...
			historySize = nativeGetHistorySize(mNativePtr);
			{
				...
				h = 0;
				{
...

					{
						...
						i = 0;
						{
							nativeGetPointerCoords(mNativePtr, map[i],
									historyPos, pc[i]);
						} // End block
					} // End collapsed parenthetic
					final long eventTimeNanos;
					eventTimeNanos = nativeGetEventTimeNanos(mNativePtr,
							historyPos);
					{
						ev.mNativePtr = nativeInitialize(ev.mNativePtr,
								nativeGetDeviceId(mNativePtr),
								nativeGetSource(mNativePtr), newAction,
		...
		pointerCount = getPointerCount();
		{
			...
			i = 0;
			{
				msg.append(", id[").append(i).append("]=")
						.append(getPointerId(i));
				msg.append(", x[").append(i).append("]=").append(getX(i));
				msg.append(", y[").append(i).append("]=").append(getY(i));
				msg.append(", toolType[").append(i).append("]=")
						.append(toolTypeToString(getToolType(i)));
			} // End block
		} // End collapsed parenthetic
		msg.append(", buttonState=").append(
				MotionEvent.buttonStateToString(getButtonState()));
		msg.append(", metaState=").append(
				KeyEvent.metaStateToString(getMetaState()));
		msg.append(", flags=0x").append(Integer.toHexString(getFlags()));
		msg.append(", edgeFlags=0x")
				.append(Integer.toHexString(getEdgeFlags()));
