 ...
 * 
 * Allows hotspots to be identified, and (after executing analysis) to be
 * queried to get a regular expression representation.
 * 
...
public class JSAStrings {
	 ...
	 * @param arg
	 *            The argument index of the hotspot.
	 */
	public void addArgumentHotspots(String signature, int arg) {
		List<ValueBox> sigSpots = StringAnalysis.getArgumentExpressions(
				signature, arg);
		logger.debug("For signature " + signature + " got " + sigSpots.size()
				+ "hotspots.");
	 ...
	 * 
	 * @param v
	 *            The Soot Value assocatied with the hotspot.
	 * @return
	 */
	...
	public void log() {
		...
		for (ValueBox v : singleton.getHotspots()) {
			...
			logger.debug("Hotspot: " + sa.getSourceFile(v) + ":::"
					+ sa.getLineNumber(v) + ":::"
	 ...
	 */
	/*
	 * private void example() { List<ValueBox> hs = getHotspots(); for(ValueBox
	 * vb : hs) { getSourceFile(vb); getSourceLine(vb); getRegex(vb.getValue());
	 * } }
	 */
