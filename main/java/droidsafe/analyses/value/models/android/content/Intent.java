 ...
 * @author dpetters
 */
public class Intent extends ValueAnalysisModeledObject {

    public Set<String> mAction = new ValueAnalysisModelingSet<String>();
    public Uri mData;
    ...
    public <T> Intent putExtra(Set<String> names, Set<T> values) {
        ...
        for(String name : names) {
            for(T value : values) {
                if      (value instanceof String) mExtras.putString(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                    new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Boolean) mExtras.putBoolean(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                      new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Byte) mExtras.putByte(new ValueAnalysisModelingSet(Sets.newHashSet(name)),
                                                                new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Character) mExtras.putChar(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                     new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Double) mExtras.putDouble(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                    new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Float) mExtras.putFloat(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                  new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Integer) mExtras.putInt(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                  new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Long) mExtras.putLong(new ValueAnalysisModelingSet(Sets.newHashSet(name)), 
                                                                new ValueAnalysisModelingSet(Sets.newHashSet(value)));
                else if (value instanceof Short) mExtras.putShort(new ValueAnalysisModelingSet(Sets.newHashSet(name)),
                                                                  new ValueAnalysisModelingSet(Sets.newHashSet(value)));
            }
        }
        return this;
